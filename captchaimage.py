...
DEFAULT_FONTS = [os.path.join(DATA_DIR, 'Oxygen-Sans.ttf')]
...
class ImageCaptcha(_Captcha):
    ...
    def create_captcha_image(self, chars, color, background):
        ...
        def _draw_character(c):
            ...
            mask, offset = font.getmask2(c, "L")
            Draw(im).text((dx-offset[0], dy-offset[1]), c, font=font, fill=color)
            # rotate
...
if __name__ == '__main__':
    image = ImageCaptcha()
    for i in range(100):
        label = ''.join(random.sample(string.ascii_letters + string.digits, 4))
        print label
        image.write(label, 'captchas/pycaptcha/'+str(i)+'.png')
